...
export default function HomeRoute() {
  ...
  return (
    <div>
      ...
      {data.state === 'loaded' && (
        <div>
          {data.data.repos.slice(0, 10).map((repo, i) => {
            return (
              <div key={i}>
                {repo.index.id} {repo.index.name}
                <br />
                excerpt: {repo.excerpt.state}<br/>
                details: {repo.details.state}
              </div>
            )
          })}
        </div>
  ...
  //           >
  //             {data.map(repo => <Project repo={repo} key={repo.id} />)}
  //           </ul>
  //         </div>
  //       </div>
  //     )
